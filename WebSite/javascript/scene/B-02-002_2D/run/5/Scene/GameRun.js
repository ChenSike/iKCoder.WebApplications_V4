	...
	for (var actionKey in i_actionPool) {
		...
		if (actionObject != null) {
			...
			if (checkResult.type == "1") {
				...
				IKCoderSenceRun_Animation_LoopBackground();
				//Scene.stepComplete();
				return;
			}
...
function IKCoderSenceRun_InitResources() {
	...
	var texture = PIXI.Texture.fromFrame(i_ip4BGtile_img);
    tilingSprite = new PIXI.TilingSprite(texture, i_background_default_width, i_background_default_height);

	i_stage.addChild(tilingSprite);
...
function IKCoderSenceRun_Animation_LoopBackground() {
	tilingSprite.tilePosition.x += 7;
	i_GameRender.render(i_stage);
	requestAnimationFrame(IKCoderSenceRun_Animation_LoopBackground);
};
