...
var _gCircleUsers = [];
var _gCircleGroups = [];
var _gCurrentChatter = { id: '', type: '', chat: '' };
...
function circleInitChats(doc) {
    ...
    for (var i = 0; i < items.length; i++) {
        ...
        for (var j = 0; j < _gCircleChats.length; j++) {
            if (_gCircleChats[j].chatId == itemObj.attr('symbol')) {
                exist = true;
                break;
            }
        }
    ...
    if (hasSysChat) {
        ...
        webSocketSend('Action_Set_NewDialog', { symbol: '', msg: '', targets: ['-1'], values: {}, batch: [], id: '' });
    }
};
...
function circleBuildFriendItem(tmpHTMLArr, item, chatId) {
    ...
    if (item.type == 'group') {
        ...
        for (var j = 0; j < item.items.length && j < 9; j++) {
            ...
            tmpImg = (item.items[j].header != 'image/tmpheader.jpg' ? _getRequestURL(_gURLMapping.account.getuserheaderImg, { uname: item.userId }) : item.items[j].header);
            tmpHTMLArr.push('           <img class="circle-group-item-header" src="' + tmpImg + '" style="' + imgStyle + marginStyle + '">');
        }

        tmpHTMLArr.push('       </div>');
    } else {
        tmpImg = (item.header != 'image/tmpheader.jpg' ? _getRequestURL(_gURLMapping.account.getuserheaderImg, { uname: item.userId }) : item.header);
        tmpHTMLArr.push('       <img class="img-fluid circle-user-list-item-header" src="' + tmpImg + '">');
    }
...
function circleUpdateCurrentLabel(chatterId, chatterType) {
    var currChatterObj = circleGetUserObj(chatterId, chatterType);
    if (chatterType == 'group' && currChatterObj.userName == '') {
        var tmpNameArr = [];
        for (var j = 0; j < currChatterObj.items.length; j++) {
            tmpNameArr.push(currChatterObj.items[j].userName);
        }
    ...
    $('.label-circle-message-history-user').attr('data-target', chatterId);
    $('.label-circle-message-history-user').attr('data-type', currChatterObj.type);
}
...
function initEvents_Circle_Chat() {
    ...
    $('.circle-message-input-send').on('click', function () {
        if (!circleCheckEmptyInput()) {
            ...
            var msg = circleBuildMessageItem(input, 0, _gUserInfoObj);
            var chatterId = $('.label-circle-message-history-user').attr('data-target');
            var symbol = '';
            for (var i = 0; i < _gCircleChats.length; i++) {
                if (_gCircleChats[i].chatter == chatterId) {
                    symbol = _gCircleChats[i].chatId;
                    _gCircleChats[i].msgs.push(msg);
                }
            }

            $('.container-circle-message-history').append(msg);
            webSocketSend('Action_Set_SendMessage', { symbol: symbol, msg: $.base64.atob(input), targets: [chatterId], values: {}, batch: [], id: '' });
        }
    });
    ...
    $('.setting-bar-chatter .row-show-more .col div').on('click', function (eventObj) {
        ...
        var groupObj = circleGetUserObj(groupId, 'group');
...
function circleBuildUserPop(eventObj) {
    if ($('.user-popover-wrap').length > 0 && $('.user-popover-wrap').css('display') != 'none') {
        ...
        if ($('.user-popover-wrap').length <= 0) {
            ...
            tmpHTMLArr.push('               </button>');
            tmpHTMLArr.push('               <button type="button" class="btn btn-sm btn-user-popover btn-share-friend" title="推荐朋友" data-target="">');
...
function circleShowSettingBar(userId, userType) {
    ...
    if (settingBar.css('display') != 'none') {
        ...
        var userObj = (userType == 'group' ? _gUserInfoObj : circleGetUserObj(userId, userType));
        ...
        if (userType == 'group') {
            var groupObj = circleGetUserObj(userId, 'group');
            circleShowSBGroupMember(groupObj, 8);
...
function circleBuildSection_Chat(id, type) {
    ...
    _gCurrentChatter.chat = '';
    for (var i = 0; i < _gCircleChats.length; i++) {
        if (_gCircleChats[i].chatter == chatterId) {
            _gCurrentChatter.chat = _gCircleChats[i].chatId;
            break;
        }
    }

    if (_gCurrentChatter.chat != '') {
        webSocketSend('Action_Get_DialogContent', { symbol: _gCurrentChatter.chat, msg: '', targets: [], values: {}, batch: [], id: '' });
    } else {
        webSocketSend('Action_Set_NewDialog', { symbol: '', msg: '', targets: [chatterId], values: {}, batch: [], id: '' });
    }
...
function circleClickAddressItem(eventObj) {
    ...
    if (userId == '-2') {
        ...
        if (currentTarget.attr('data-type') == 'group') {
            ...
            $('.row-circle-message-part-1').show();
            $('.row-circle-message-part-2').hide();
            currUserObj = circleGetUserObj(userId, 'group');
            circleBuildGroupDetail(userId);
        } else {
            currUserObj = circleGetUserObj(userId, currType);
...
function circleBuildNewFriendsList() {
    ...
    $('.circle-address-new-friend-header').on('click', function (eventObj) {
        var userObj = circleGetUserObj($(eventObj.currentTarget).attr('data-target'), 'user');
...
function circleBuildFriendDetail(userObj) {
    if (userObj != null) {
        ...
        if ($('.container-circle-friend-item-detail').length == 0) {
            ...
            tmpHTMLArr.push('           <button type="button" class="btn btn-sm btn-circle-friend-item-detail-replay" title="回复" data-target="">');
...
function circleSwitchToTalk(chatterId, chatterType) {
    ...
    circleUpdateCurrentLabel(chatterId, chatterType);
    $('.col-circle-message-history-button .btn.btn-sm').attr('data-target', chatterId);
    $('.col-circle-message-history-button .btn.btn-sm').attr('data-type', chatterType);
    _gCurrentChatter.id = chatterId;
    _gCurrentChatter.type = chatterType;
    for (var i = 0; i < _gCircleChats.length; i++) {
        if (_gCircleChats[i].chatter == chatterId) {
...
function circleBuildGroupDetail(groupId) {
    var currGroup = circleGetUserObj(groupId, 'group');
    var container = $('.container-circle-message-history');
    var tmpHTMLArr = [];
...
function circleClickUserItem(currentTarget) {
    ...
    circleUpdateCurrentLabel(tmpSymbol, tmpType);
    ...
    symbolEl.text('0');
    symbolEl.hide();
    webSocketSend('Action_Get_DialogContent', { symbol: tmpChatId, msg: '', targets: [], values: {}, batch: [], id: '' });
};
...
var _gCurrentAction = '';
//iSymbol, iMsg, iTargets, iValue, iBatch
...
function webSocketReceiveCircle(evt) {
        ...
        case 'Action_Get_RelationsList':
            if (passive) {
                ...
                if (addressItem.hasClass('active')) {
                    webSocketSend('Action_Get_BatchArrProfile', { symbol: '', msg: '', targets: [], values: {}, batch: getUesrIdArrFromNewFriendDoc(valDoc), id: '' });
                } else {
    ...
    var currUserObj = circleGetUserObj(currUserSymbol, 'user');
