...
(function () {
    ...
    if (!window.methodDraw) window.methodDraw = function ($) {
        ...
        Editor.init = function () {
          ...
          "none", "#ffffff", "#cccccc", "#999999", "#333333", "#000000"
            ],
            ...
            var setupFlyouts = function (holders) {
                $.each(holders, function (hold_sel, btn_opts) {
                    ...
                    shower.mousedown(function (evt) {
                        ...
                        // Tynker no delay before showing popup (no need to tap and hold)
                        var time = 0;
            ...
            var extAdded = function (window, ext) {
                ...
                if (ext.buttons) {
                    ...
                    $.each(ext.buttons, function (i, btn) {
                        ...
                        if (!svgicons) {
                            icon = (btn.type == "menu") ? "" : (btn.icon ? $('<img src="' + btn.icon + '">') : "");
                        } else {
            ...
            if (!document.getElementById('canvas_background') && curConfig.addBackground) createBackground();
            var fill = document.getElementById('canvas_background') && document.getElementById('canvas_background').getAttribute("fill");
            ...
            // Tynker customized color palette (position transparent using none color string)
            var str = '';
            palette.forEach(function (item, i) {
                if (item == "none") {
                    str += '<div class="palette_item transparent" style="border-color: #ddd;"data-rgb="' + item + '"></div>';
                    return;
                }
        ...
        .on('mouseover', function () {
			...
			*/
            ...
            var colorPicker = function (elem) {
                ...
                var was_none = false;
                // Tynker customized position
                var pos = is_background ? { 'right': 175, 'top': 50 } : { 'right': 175, 'top': 50 }
            ...
            var Actions = function () {
                ...
                var tool_buttons = [
					...
					// Tynker new undo button
					{ sel: '#tool_undo,#tool_undo_button', fn: clickUndo, evt: 'click', key: modKey + 'z' },
					{ sel: '#tool_redo,#tool_redo_button', fn: clickRedo, evt: 'click', key: ['y', true] },
            ...
            // Tynker exports
            Editor._clickSelect = clickSelect;
            Editor._updateContextPanel = updateContextPanel;
            Editor._changeCanvasSize = changeCanvasSize;
        };
    ...
    // SVG-edit adds the svgEditor global var, but Method-Draw uses methodDraw,
    // this allows unmodified svg-edit extensions to still run.
    window.svgEditor = methodDraw;
