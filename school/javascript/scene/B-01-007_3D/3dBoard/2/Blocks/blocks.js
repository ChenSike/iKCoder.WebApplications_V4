...
Blockly.Blocks['positive_rotate_diamond'] = {
    init: function () {
        this.appendDummyInput()
        .appendField(new Blockly.FieldDropdown([["顺时针","positive"], ["逆时针","negative"]]), "rotate_derection")
        .appendField("旋转菱形")
...
Blockly.JavaScript['positive_rotate_diamond'] = function (block) {
    var a = block.getFieldValue('rotate_derection');
    if (a == 'positive'){
        return 'Scene.RotateDiamond(' + block.getFieldValue('rotate_degree') + ',false);\n';
    }else if (a == 'negative')
        return 'Scene.RotateDiamond(' + block.getFieldValue('rotate_degree') + ',true);\n';
};

Blockly.Blocks['negative_rotate_diamond'] = {
    init: function () {
        this.appendDummyInput()
        .appendField(new Blockly.FieldDropdown([["逆时针","negative"] ,["顺时针","positive"]]), "rotate_derection2")
        .appendField("旋转菱形")
...
Blockly.JavaScript['negative_rotate_diamond'] = function (block) {
    var a = block.getFieldValue('rotate_derection2');
    if (a == 'positive'){
        return 'Scene.RotateDiamond(' + block.getFieldValue('rotate_degree2') + ',false);\n';
    }else if (a == 'negative')
        return 'Scene.RotateDiamond(' + block.getFieldValue('rotate_degree2') + ',true);\n';
};
...
Blockly.JavaScript['draw_diamond'] = function (block) {
    return 'draw_diamond();\n';
};

Blockly.Blocks['draw_diamond_function'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("函数    画菱形");
    this.appendStatementInput("do")
...
Blockly.JavaScript['draw_diamond_function'] = function(block) {
  var statements_do = Blockly.JavaScript.statementToCode(block, 'do');
  var code = 'function draw_diamond()\n {\n' + statements_do + ' Scene.Brush.groupPatterns(4);\n};\n';
  return code;
};